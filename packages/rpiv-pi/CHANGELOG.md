...
- **`discover` skill (FRD-producer role)** (`packages/rpiv-pi/skills/discover/SKILL.md` + `templates/frd.md`): refills the slot emptied by `0a3e1e2` with a fundamentally different role — interview-driven Feature Requirements Document producer modeled after the source intent skill (`~/Downloads/intent/SKILL.md`). One-question-at-a-time interview with a recommended answer at every step, light agent fan-out (≤2 agents: `codebase-locator` + optionally `codebase-analyzer`) for grounding, and a flat-timestamp artifact at `thoughts/shared/discover/<YYYY-MM-DD_HH-MM-SS>_<topic>.md`. Each invocation writes a fresh artifact (no slug-collision detection); input may be free-text or an existing artifact path for refinement. The FRD's `Decisions` block is translated into research's Developer Context per the chain integration below, then inherited by `design` via the existing contract at `packages/rpiv-pi/skills/design/SKILL.md:36`. **Note**: this is a NEW role for a previously-removed skill name. The OLD discover (codebase-discovery question-formulator deleted in `0a3e1e2`) is gone and stays gone; migration directives from the previous `[Unreleased]` Removed entry continue to apply for OLD-discover semantics.
- **`thoughts/shared/discover/` THOUGHTS_DIRS entry** (`packages/rpiv-pi/extensions/rpiv-core/session-hooks.ts`): added at the head of the tuple (pipeline order: discover first). First addition to `THOUGHTS_DIRS` since `6d8a431` (`reviews`); the inverse of `0a3e1e2`'s removal precedent.
...
- **`research` skill** (`packages/rpiv-pi/skills/research/SKILL.md`): Step 1 gains a sub-step recognizing a mentioned `thoughts/shared/discover/*.md` path. When present, the FRD is read FULLY before scope-tracer dispatch; each `### Decision` block translates into a `**Q (discover: <title>): <Question>**` + `A: <Chosen>` Developer Context entry; the FRD's `Recommended Approach` text becomes the scope-tracer topic; Open Questions carry forward verbatim. Plain free-text input continues to dispatch scope-tracer unconditionally — `argument-hint` stays free-text-only. Important Notes gains one bullet documenting the chain.
- **`rpiv-site` PIPELINE tuple + `Pipeline.astro`** (`packages/rpiv-site/src/lib/skills.ts`, `packages/rpiv-site/src/components/Pipeline.astro`, `packages/rpiv-site/src/content/skills/discover.md`): pipeline restored to 6 nodes (`discover` at head); kicker `5 nodes` → `6 nodes`; header `five-step` → `six-step`; aria-label adds `discover`; x-position spacing formula `584 / 4` → `584 / 5`. `ARTIFACT_WRITE_SITES["discover"]` set to `"thoughts/shared/discover/"`. Content stub `packages/rpiv-site/src/content/skills/discover.md` restored with FRD-producer tagline (distinguishes from yesterday's deleted stub which carried codebase-discovery role copy).
...
- **Slot refill, not slot revert**: `/skill:discover` returns with a NEW role (interview-driven FRD producer), not the OLD role (codebase-discovery question-formulator). Custom skills that previously referenced `/skill:discover` for question-formulation are STILL out per the migration directive in the existing Removed entry above; they should call `/skill:research "<topic>"` directly. The new `/skill:discover` is for human-driven intent capture before research runs.
- **Pipeline counter**: `rpiv-site` returns to 6 nodes. Anyone mirroring the visitor copy in their own materials should update `5 nodes` / `five-step chain` → `6 nodes` / `six-step chain` and add `discover` at the head of the pipeline list.
- **`thoughts/shared/discover/` directory**: created on first session_start after install via `THOUGHTS_DIRS` scaffold. No manual `mkdir` required.
